...
namespace RayTracerWinFormsTest
{
    class Orthogonal : ICamera
    {
        ...
        {
            this.EyePosition = eye; this.Angle = angle; this.CameraSize = size;
        }
        ...
        {   // Kierunek w którym skierowane są wszystkie promienie    
            ...
            // położenia kamery    
            Vector2 offsetFromCenter = new Vector2(pictureLocation.X * CameraSize.X, pictureLocation.Y * CameraSize.Y);
            // Obliczenie finalnego położenia kamery,    // rówież proste funkcje trygonometryczne.    
            Vector3 position = new Vector3(EyePosition.X + offsetFromCenter.X * Math.Cos(Angle), EyePosition.Y + offsetFromCenter.Y, EyePosition.Z + offsetFromCenter.X * Math.Sin(Angle));
